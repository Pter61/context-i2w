...
def get_text_features_replace_test(model, token_features, replaced_texts, args):
...
def get_text_features_replace(model, token_features, replaced_texts, args):
    # text = tokenize("a photo of")
    # print(text)
    text = replaced_texts.cuda(args.gpu, non_blocking=True)
    # text = text.view(1, -1)
    # text = text.repeat(token_features.size(0), 1)
    text_features = model.encode_text_img_replace(text, token_features)
    return text_features
...
def get_loss_img2text_replace(model, img2text, images, loss_img, loss_txt, args, replaced_texts, memory=None):
    with torch.no_grad():
        image_features = model.encode_image(images)
    token_features = img2text(image_features)
    text_features = get_text_features_replace(model, token_features, replaced_texts, args)
    image_features = image_features / image_features.norm(dim=-1, keepdim=True)
    text_features = text_features / text_features.norm(dim=-1, keepdim=True)
    logit_scale = model.logit_scale.exp()
    logit_scale = logit_scale.mean()
...
def get_loss_img2text_replace_token(model, img2text, images, loss_img, loss_txt, args, replaced_texts, memory=None):
    text = replaced_texts.cuda(args.gpu, non_blocking=True)
    # print(text)
    with torch.no_grad():
        image_features = model.encode_image(images)
        image_token_features = model.encode_image_tokens(images)
        # print(image_features.shape, image_token_features.shape) # torch.Size([1, 768]) torch.Size([1, 257, 1024])
        text_cls_embeddings = model.encode_text(text)

    # token_features = img2text(image_features)
    token_features = img2text(text_cls_embeddings, image_token_features)
    text_features = get_text_features(model, token_features, args)
    # text_features = get_text_features_replace(model, token_features, replaced_texts, args) get_text_features
...
def train(model, img2text, data, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        images, texts, replaced_texts = batch[0], batch[1], batch[2]
        ...
        if args.precision == "amp":
            with autocast():
                total_loss = get_loss_img2text_replace_token(m, img2text, images, loss_img, loss_txt, args, replaced_texts, data_identifier)
