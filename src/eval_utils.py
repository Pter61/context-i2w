...
def evaluate_imgnet_retrieval(model, img2text, args, prompt, query_loader, target_loader):
    ...
    with torch.no_grad():
        ...
        # print(all_target_image_paths, len(all_target_image_paths)) # [..., './CIR_change/data/imgnet/real/n02398521/ILSVRC2012_val_00027980.JPEG'] 16983
        ...
        for p_ind, p in enumerate(prompt):            
            ...
            for batch in tqdm(query_loader):
                ...
                image_features = m.encode_image(images)
                query_image_token_features = model.encode_image_tokens(images)
                 ## Composed feature extraction

                b_size = query_image_token_features.shape[0]
                text_all = text.repeat(b_size, 1)
                text_cls_embeddings = model.encode_text(text_all)
                # image_features_query = img2text(image_features)
                image_features_query = img2text(text_cls_embeddings, query_image_token_features)
            ...
            # print(all_source_image_paths, len(all_source_image_paths)) # 【..., './CIR_change/data/imgnet/real/n02398521/ILSVRC2012_val_00033265.JPEG'] 10000
...
def evaluate_coco(model, img2text, args, loader):
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            query_image_token_features = model.encode_image_tokens(region_images)
            text_cls_embeddings = model.encode_text(text_with_blank_query)
            # query_image_tokens = img2text(query_image_features)
            query_image_tokens = img2text(text_cls_embeddings, query_image_token_features)
...
def evaluate_cirr(model, img2text, args, query_loader, target_loader):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(query_loader): # 66
            ...
            query_image_token_features = model.encode_image_tokens(ref_images)
            text_cls_embeddings = model.encode_text(text_with_blank)
            # query_image_tokens = img2text(query_image_features)
            query_image_tokens = img2text(text_cls_embeddings, query_image_token_features)
...
def evaluate_fashion(model, img2text, args, source_loader, target_loader):
    ...
    with torch.no_grad():
        for batch in tqdm(source_loader):
            ...
            text_cls_embeddings = model.encode_text(target_caption)
            id_split = tokenize(["<|replace|>"])[0][1]
...
def get_metrics_imgnet(query_features, image_features, query_labels, target_labels, source_image_paths, target_image_paths, exp_type, p_ind):
    metrics = {}
    num_classes = 7000
    ...
    batches = [(query_features[x:x+100], query_onehot[x:x+100], source_image_paths[x:x+100]) for x in range(0, len(query_features), 100)]
